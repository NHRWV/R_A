...
url <- read.csv("googleplaystore.csv", header = T, stringsAsFactors = F, na.strings ="NaN")
str(url)
...
```
  t값이 신뢰구간에서 벗어나 있다. 뿐만 아니라 p-value값이 신뢰수준보다 매우 낮게 나왔다. 따라서 카테고리에 따른 레이팅의 차이가 분명 존재함을 암시한다. 영가설을 기각한다.
